...
#define RAISE_TYPE_ERROR(vm, op, a, b)                                                           \
    ...
    char buffer[256];                                                                            \
    snprintf(buffer, sizeof(buffer), "invalid operand types for operation (%s) : '%s' and '%s'", \
             op, lhs_type_name, rhs_type_name);                                                  \
...
static inline struct lu_value lu_vm_op_add(struct lu_vm* vm, struct lu_value a, struct lu_value b) {
    ...
    if (lu_is_string(a) || lu_is_string(b)) {
        return lu_value_object(lu_cast(struct lu_object, lu_string_concat(vm->istate, a, b)));
    }
