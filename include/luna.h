...
#define LU_RETURN_INT(x) return ((lu_value){.type = VALUE_INTEGER, .integer = (x)})
#define LU_RETURN_BOOL(x) return ((lu_value){.type = VALUE_BOOL, .integer = (x)})
#define LU_RETURN_OBJ(x) return ((lu_value){.type = VALUE_OBJECT, .object = (x)})
...
#define LU_TRY_UNPACK_INT(vm, args, idx, dest)                                                   \
    do {                                                                                         \
        if (!lu_is_int((args)[(idx)])) {                                                         \
            ...
            char buffer[256];                                                                    \
            snprintf(buffer, sizeof(buffer), "bad argument #%d: expected '%s', got '%s'", (idx), \
...
#define LU_TRY_UNPACK_ARGS(vm, typespec, argc, args, ...)                                  \
        ...
        for (__i = 0; __i < argc; __i++) {                                                 \
            switch (__t[__i]) {                                                            \
                ...
                case 's': {                                                                \
                    ...
                    *((struct lu_string**)(__args[__i])) = lu_as_string(args[__i]);        \
                    break;                                                                 \
                }                                                                          \
                case '?': {                                                                \
    ...
    arg_raise_type_error:                                                                  \
        ...
        const char* expected_type_str_name = get_type_name_from_mnemonics(__t[__i]);       \
        char buffer[256];                                                                  \
        snprintf(buffer, sizeof(buffer), "bad argument #%d : expected '%s' got '%s'", __i, \
                 expected_type_str_name, got_type_str_name);                               \
        lu_raise_error(vm->istate, buffer);                                                \
        return lu_value_none();                                                            \
...
#define LU_REGISTER_NATIVE_FN_WITH_NAME(state, obj, name, func, pc) \
    lu_register_native_fn_with_name(state, obj, state->names.name, func, pc)
