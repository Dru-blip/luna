...
fn parseClassDecl(p: *Parser) ParserError!*Node {
    ...
    var super_class_token: ?*const Token = null;

    var super_class: ?[]const u8 = null;
    if (p.peek().tag == .colon) {
        p.advance();
        super_class_token = try p.expectToken(.identifier);
        super_class = p.source[super_class_token.?.loc.start..super_class_token.?.loc.end];
    }
...
fn parsePrefixExpr(p: *Parser) ParserError!*Node {
    ...
    switch (token.tag) {
        .bang => {
            p.advance();
            const arg = try p.parsePrefixExpr();
            const node = try p.ast.makeNode(.not, token.loc.merge(&arg.loc));
            node.data = .{
                .un = arg,
            };
            return node;
        ...
        .plus => {
            p.advance();
            const arg = try p.parsePrefixExpr();
            const node = try p.ast.makeNode(.un_plus, token.loc.merge(&arg.loc));
            node.data = .{
                .un = arg,
            };
            return node;
        ...
        .minus => {
            p.advance();
            const arg = try p.parsePrefixExpr();
            const node = try p.ast.makeNode(.un_negate, token.loc.merge(&arg.loc));
            node.data = .{
                .un = arg,
            };
            return node;
        ...
        .l_brace => {
            ...
            while (p.peek().tag != .r_brace) {
                ...
                const property = try p.ast.makeAttribute(key.loc.merge(&value.loc), key, value);
