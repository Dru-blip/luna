...
static struct token next_token(struct tokenizer *t) {
    ...
    case '+':
        advance(t);
        kind =
            (current(t) == '=') ? (advance(t), TOKEN_PLUS_EQUAL) : TOKEN_PLUS;
        break;
    case '-':
        advance(t);
        kind =
            (current(t) == '=') ? (advance(t), TOKEN_MINUS_EQUAL) : TOKEN_MINUS;
        break;
    case '*':
        advance(t);
    ...
    case '/':
        advance(t);
        kind =
            (current(t) == '=') ? (advance(t), TOKEN_SLASH_EQUAL) : TOKEN_SLASH;
        break;
    case '%':
        advance(t);
    ...
    case '=':
        advance(t);
        kind =
            (current(t) == '=') ? (advance(t), TOKEN_EQUAL_EQUAL) : TOKEN_EQUAL;
        break;
    case '!':
        advance(t);
        kind =
            (current(t) == '=') ? (advance(t), TOKEN_BANG_EQUAL) : TOKEN_BANG;
        break;
    case '<':
        advance(t);
        if (current(t) == '=') {
    ...
    case '"': {
        ...
        while (true) {
            ...
            if (c == '"') {
                ...
                fprintf(
                    stderr,
                    "Unterminated string literal starting at line %d, col %d\n",
                    t->line, t->col);
                exit(1);
            } else {
                advance(t);
